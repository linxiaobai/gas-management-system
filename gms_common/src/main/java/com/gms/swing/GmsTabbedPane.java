 ...
 * 可关闭TabbedPane的实现
 */
public class GmsTabbedPane extends JTabbedPane implements MouseListener {
    ...
    public GmsTabbedPane() {
        super();
        addMouseListener(this);
    }

    public GmsTabbedPane(int tabPlacement, int tabLayoutPolicy) {
        super(tabPlacement, tabLayoutPolicy);
        addMouseListener(this);
    }

    public GmsTabbedPane(int tabPlacement) {
        super(tabPlacement);
        addMouseListener(this);
    }
...
//        } else
    ...
    private void showPopupMenu(final MouseEvent event) {
        ...
        if (event.isPopupTrigger()) {
            final int index = ((GmsTabbedPane)event.getComponent()).getUI().tabForCoordinate(this, event.getX(), event.getY());
            final int count = ((GmsTabbedPane)event.getComponent()).getTabCount();
            JPopupMenu pop = new JPopupMenu();
            JMenuItem closeCurrent = new JMenuItem("关闭当前");
            closeCurrent.addMouseListener(new MouseAdapter() {
                public void mouseReleased(MouseEvent e) {
                    ((GmsTabbedPane)event.getComponent()).removeTabAt(index);
                }
            });
            ...
            closeLeft.addMouseListener(new MouseAdapter() {
                public void mouseReleased(MouseEvent e) {
                    for(int j=(index-1);j>=0;j--) {
                        ((GmsTabbedPane)event.getComponent()).removeTabAt(j);
                    }
                }
            });
            pop.add(closeLeft);
